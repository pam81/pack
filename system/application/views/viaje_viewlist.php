  ...
  <div class="select-bar">
    <form name="formsearch" id="formsearch" method="post" action="<?php echo site_url() . "viaje/index"; ?>" onsubmit="return flete.validarSearch(3);">
      <label> <?php echo mb_convert_case($this->lang->line("title_movil"), MB_CASE_TITLE, "utf-8"); ?>
        <input size="10" type="text" tabindex="1" name="searchfield" id="searchfield" value="<?php if (isset($movil)) echo $movil;
                                                                                              else echo 0; ?>" onkeypress="searching(event); " />
      </label>
      ...
      <label>
        ...
        <input type="checkbox" tabindex="9" name="checkpendiente" value="1" <?php if (isset($pendiente) && $pendiente) echo "checked"; ?> />
        pendiente
      </label>
    ...
    <tbody>
      ...
      foreach ($viajes as $u) { ?>
        <tr class="modotr">
          ...
          <td> <?php if ($u->forma_pago == 1) {
                  echo $total;
                } ?> </td>
          <td> <?php if ($u->forma_pago == 2) {
                  echo $total_ctacte;
                } ?> </td>
          <td>
